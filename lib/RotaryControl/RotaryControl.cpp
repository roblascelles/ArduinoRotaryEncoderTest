...
RotaryControl::RotaryControl(int clk, int dt, int sw, turnCallback turnCallback, pressCallback pressCallback)
{
    _clkPin = clk;
    _dtPin = dt;
    _swPin = sw;
...
{
    ITimer.attachInterruptInterval(HW_TIMER_INTERVAL_MS * 1000L, RotaryControl::pressedInterupt);
}
...
void RotaryControl::pressedInterupt()
{
    for (int i = 0; i < controlCnt; i++)
    {
        controls[i]->pressed();
